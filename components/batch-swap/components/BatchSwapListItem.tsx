...
export function BatchSwapListItem({
  tokenIn,
  tokenOut,
  tokenInPrice,
  tokenOutPrice,
  ...
  const percentChange =
    tokenInPrice !== 0 ? (tokenOutPrice / tokenInPrice - exchangeRate) / exchangeRate : 0;
  ...
  return (
    <Card mb="1" className="sor" bgColor="vertek.slatepurple.900">
      ...
      <Flex>
        ...
        <Box>
          <Badge>
            {formatDistanceToNow(new Date(batchSwap.timestamp * 1000), { addSuffix: true })}
          </Badge>
        </Box>
      </Flex>
      <Box height="64px">
