...
export function poolGetRequiredToken(
  address: string,
  ...
  try {
    return SDK.StableMath._calcBptOutGivenExactTokensIn(
      ...
      pool.tokens.map((poolToken) => {
        ...
        return tokenAmount
          ? scaleTo18AndApplyPriceRate(tokenAmount.amount, poolToken)
          : oldBnumZero();
      }),
...
export function poolWeightedBptForTokensZeroPriceImpact(
  ...
  const tokenBalancesScaled = pool.tokens.map((token) =>
    oldBnumScaleAmount(token.balance, token.decimals),
  );
  const tokenWeightsScaled = pool.tokens.map((token) =>
    oldBnumScaleAmount(token.weight || '0', 18),
  );
...
export function scaleTo18AndApplyPriceRate(
...
export function removePriceRateFromAmount(
...
export function poolBatchSwaps(
  assets: string[][],
  swaps: SwapV2[][],
...
export function poolStableExactBPTInForTokenOut(
  ...
  const scaledTokenAmountOut = scaleTokenAmountDownFrom18Decimals(
    token,
    tokenAmountOut,
...
export function poolGetNestedStablePoolTokens(
...
export function poolGetMainTokenFromLinearPoolToken(
  linearPoolToken: GqlPoolTokenLinear,
): GqlPoolToken {
  const mainToken = linearPoolToken.pool.tokens.find(
    (token) => token.index === linearPoolToken.pool.mainIndex,
  );
...
export function poolGetWrappedTokenFromLinearPoolToken(
  linearPoolToken: GqlPoolTokenLinear,
): GqlPoolToken {
  const wrappedToken = linearPoolToken.pool.tokens.find(
    (token) => token.index === linearPoolToken.pool.wrappedIndex,
  );
...
export function poolGetTotalShares(
  ...
  return pool.__typename === 'GqlPoolPhantomStableNested'
    ? pool.totalShares
...
export function poolGetNestedTokenEstimateForPoolTokenAmounts({
  ...
  let tokenAmountsOut = poolTokenAmounts.filter((amountOut) =>
    nestedTokens.includes(amountOut.address),
  );
  let bptAmounts = poolTokenAmounts.filter(
    (amountOut) => !nestedTokens.includes(amountOut.address),
  );
  ...
  for (const nestedStablePoolToken of nestedStablePoolTokens) {
    ...
    const bptAmount = bptAmounts.find(
      (amount) => amount.address === nestedStablePoolToken.address,
    )!;
...
export function tokenAmountsAllZero(tokenAmounts: TokenAmountHumanReadable[]) {
  return (
    tokenAmounts.filter((amount) => parseFloat(amount.amount) === 0).length === tokenAmounts.length
  );
}
